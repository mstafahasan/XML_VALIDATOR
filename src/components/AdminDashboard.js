...
import { 
  Mail, Eye, CheckCircle, Clock, User, MessageSquare, 
  LogIn, LogOut, BarChart3, Settings, Reply, Trash2,
  Filter, Search, Plus, AlertCircle, TrendingUp
} from 'lucide-react';
  ...
  const updateMessage = async (messageId, updates) => {
    try {
      ...
      if (response.ok && result.success) {
        ...
        alert(`Error: ${result.error || 'Failed to update message'}`);
      }
    } catch (error) {
  ...
  const handleDelete = async (messageId) => {
    ...
    try {
      ...
      if (response.ok && result.success) {
        ...
        alert(`Error: ${result.error || 'Failed to delete message'}`);
      }
    } catch (error) {
